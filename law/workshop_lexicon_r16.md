...
**Revision identity: rev 16** — the rev counter increments by one on every
...
(`workshop_lexicon_r15.md`), per the rev-in-filename law ratified
2026-07-13d [operator-ratified]: filed files are immutable, every re-issue
mints a new filename (this file is `workshop_lexicon_r15.md`), current =
...
| `pipeline_config_schema.md` | The form every source fills out; §1 fences engine constants; §3.2.1 per-source v2 blocks (chunk / arbitration / ingest / web_policy et al.); §6 wire schema v0; Appendix C amendment record | **v2 RATIFIED 2026-07-13b** (deviations docket 1–13, operator wholesale after arguments pass). W10 closed as change 0 — the 07-12c patch re-applied verbatim-to-spec inside the re-issue. **Re-file OWED as `pipeline_config_schema_v2.md`** (rev-in-filename law, 07-13d): the operator files the 07-13b session download under the NEW name — retires W9 lag #4 permanently (the mount served stale v1 across three sessions under the old same-name scheme) |
| `discovery_prompt_v2_5.md` | Current discovery prompt (amendment v1.2 item 8 applied: ambiguity probe + watchlist pass, widened arbitration axis incl. `web_policy` ruling, arbitration seed + chunk plan sidecars, exit-exam extension, `[discovery-forecast]` tag) | v2.5 ratified 07-13 (deviations docket 1–8, operator wholesale); proof pending first run; v1–v2.4 are sediment (v2.4 proof banked, Schemer 07-10). **v2.6 clerical re-version OWED** (schema-boundary section stale after schema v2: the v1-plain/v2-proposed split it hard-codes is retired; boundary section only, ideally before the next discovery run). ⚠ **NEVER RUN (recorded 07-13f, `discovery_test_record_v1.md`): every validated behavior belongs to an ANCESTOR version — v2.5's additions (ambiguity probe, watchlist pass, arbitration seed, chunk plan, exit-exam extension, sixth tag) have never executed. "Proof banked" = inherited by assertion, never re-earned (no regression has ever been run). Evidence surface = 4 ancestor runs + 6 validated behaviors; test method is live-run, NOT sealed-kit** |
...
| `primer_amendment_proposals_v1.md` | **OPEN REGISTER** of proposed changes to the primer, accumulating evidence across sessions until the operator ratifies a batch into primer v5. Holds P1–P3 verbatim (mount-currency step; manifest index cross-check; no-receipt-for-unperformed-step) + P4–P8 (read-the-runbook-before-blocking; search-before-asserting-history; honest-TBD generalized; bundled-button loudness; P1–P3 sunset-on-migration). Each proposal carries an evidence count = sessions in which the failure ACTUALLY OCCURRED, plus its strongest counter | SHIPPED 07-13f [operator-ruled: addendum instead of a unilateral primer bump]. **NOT LAW — binds nothing.** A booting ctx reads `workshop_primer_v4.md` for law and this file only to learn what is pending. **P1–P3 carry 2 evidence points each (07-13e, 07-13f) and are ready for ratification.** Standing exception: a proposal may be applied BY HAND when the current checkpoint's manifest instructs it (this is how P1–P3 protected the 07-13f boot) — a crutch that depends on the ctx reading the checkpoint and choosing to comply, NOT a substitute for ratification |
...
| `workshop_lexicon_r<N>.md` | This file (current = highest rev: **r12**, 07-13d) | reference, not authority; rev-in-filename from r12 on; `workshop_lexicon_v1.md` copies are pre-migration sediment |
| `system_diagram_r<N>.html` | End-to-end system map (interactive mermaid + wire schemas + actor status ledger + undesigned inventory, one file; current = highest rev: **r8**, 07-13d) | reference, not authority; statuses rot — newest checkpoint wins; rev-in-filename from r8 on; `system_diagram_v1.html` copies are pre-migration sediment. Retired: `system_diagram_v1.md` (superseded by the HTML, 07-10) |
...
- **W1 (recorded, unresolved by ruling 07-10):** amendment v1.1 §2 cites
  ...
  don't resolve]
- **W2:** provenance tag set is FIVE (see §6); docs predating v2.1 list three.
  Five wins.
