...
P4-ready, P11, P12);
re-issued 2026-07-18 (**Session A — clerical filing of the v2.7 vindication
evidence**: W15 minted (`coverage by silence` — the Loeliger p174–179 gap +
the rappers `ch18` cosmetic skip); `meta artifact` and `confirmer→detector`
minted in §3; §9 pair 15 `three names, one file`; register v5 files P13/P14
from their 07-15b drafts and mints P15/P16; NO engine artifact touched).
**Revision identity: rev 19** — the rev counter increments by one on every
...
**meta artifact** — an artifact whose subject is the workshop's own process
rather than the pipeline that eats books: the primer, this lexicon, the
diagram, the amendment register, the roadmap, checkpoints. Distinguished from
an **engine artifact** (a machine part — a prompt, schema, scorer) and from an
**ops output** (a per-source deliverable — a discovery pile, a harvest). Meta
artifacts are how the workshop remembers how to run itself; they ride along
with every session by definition and never count as the session's one engine
artifact. *Plain: the paperwork ABOUT how we work, as opposed to the machine
parts or the products.*

**confirmer→detector** — a discipline upgrade applied to any mandated check:
a check written only to CONFIRM a verdict already reached lets a clean-looking
defect ship (nothing triggers a second look), whereas the same check written
as a DETECTOR — run up front, before any verdict, empowered to force a
re-verification — catches the defect the confirmer would pass. Two instances on
record: v2.7 turned the raster rule from confirmer to detector (rasterize up
front, before trusting the text layer — this caught the Loeliger ■→B OCR
corruption); and 07-18 sharpened the coverage check the same way one level up
(coverage arithmetic that TRIGGERS mandatory page re-verification on a gap, not
a lint that merely fails after the fact — see §10 W15). *Plain: don't run the
test only to rubber-stamp what you already decided; run it first, and let it
change your mind.*
...
| `evidence/loeliger-til-run-2026-07-18/` | **The v2.7 VALIDATION evidence** — 6 family files + `session_report_loeliger-til.md` + `_COVERAGE_NOTE.md`. Source: R.G. Loeliger, *Threaded Interpretive Languages* (1981), 266pp scanned OCR (`InvisibleOCR` text layer), slug `loeliger-til`. The v2.7 re-run, audited cold by a third ctx | FILED 07-18 (Session A), terminal state `ratified`. Filed AS-IS with a coverage-gap note: the plan silently drops printed p174–179 (§6.2 + §6.3) — the coverage-by-silence defect (W15/P16), confirmed three independent ways, patch owed to roadmap D. **Filed WITH the defect labeled, not cleaned** — the run is evidence FOR the coverage rule. Envelope: not re-validated this session |
...
15. **three names, one file (the config-schema naming trap)** — the pipeline
config schema is referred to by THREE different filenames across the corpus and
they all mean the same single artifact: charter §9 names
`pipeline_config.schema.md` (dot), amendment v1.2 names `pipeline_config_schema.md`
(underscore), and the repo holds `pipeline_config_schema_v2.md` (underscore +
suffix, the current file). A ctx that greps for one spelling and misses the other
two can wrongly conclude the schema is absent. **The current file is the
`_v2`-suffixed one; the two unsuffixed names are legacy references, not missing
files.** *Plain: same document, three different labels stuck on it over time —
don't mistake a label mismatch for a lost file.* [mechanical: name reconciliation
across charter §9, amendment v1.2, repo listing — 07-15b]
...
- **W15 (coverage by silence — recorded 2026-07-18):** a chunk plan that
  CLAIMS complete coverage ("every in-scope range in exactly one chunk") while
  a page range silently belongs to no chunk. First real instance: the Loeliger
  TIL run dropped printed p174–179 (§6.2 "A Classy Cross-Reference" + §6.3 "Sum
  Total") — the convention row for the 172-entry dictionary ended at p173, the
  next section row began at p180, and nothing owned the gap. Root cause:
  printed p174 does double duty (dictionary end + §6.2 start) and the plan's
  boundary logic assumed one page = one owner. **Confirmed by three independent
  methods**: the third-ctx auditor's arithmetic (rows jump p173→p180), the
  07-18 raster read of the pages against the binary, and the Session-A filing
  check's own coverage-partition re-derivation. A second, HARMLESS instance is
  the rappers plan's skipped chunk-id `ch18` (a merge ate it, id never
  renumbered; coverage genuinely 219/219) — same organ, cosmetic. **The wart is
  that the defect is INVISIBLE to a reader** (the plan reads complete) and
  visible only to arithmetic. The fix is confirmer→detector (see §3): coverage
  arithmetic that TRIGGERS mandatory re-verification of affected pages, not a
  check run only to confirm a verdict already reached. Docketed as
  `docket_chunk_coverage_reverify_v1.md`; filed as register P16; the
  discovery-side rule lands in v2.8 (roadmap B.1) with a lint twin in the
  validator (roadmap C.2).
...
- 2026-07-18 — **Session A: clerical filing of the discovery-v2.7 vindication
  evidence** (Fable, first Fable workshop-adjacent ctx; an ASSISTING/advisory
  session, not a derivation — boot verification loop run as a capability probe
  per the model-handoff law, and it caught four real things: a two-rev-stale
  lexicon base, a register/primer naming slip, a missing source-checkpoint
  dependency, and the Loeliger coverage gap). NO engine artifact. Reference
  deltas: **§10 W15 minted** (`coverage by silence`); **§3 `meta artifact` +
  `confirmer→detector` minted**; **§9 pair 15** (`three names, one file`).
  Companion filings this session: register v5 (P13/P14 filed, P15/P16 minted),
  diagram r14 (discovery-v2.7-VALIDATED), and the v2.7 run evidence itself
  (Loeliger pile + report under `evidence/`, filed as-is with a coverage-gap
  note). The rappers pile was CONFIRMED already in the repo (byte-identical to `evidence/rappers-handbook-run-2026-07-15/`, verified by md5 via a P13 requisition snippet) and NOT re-filed. rev 18 → 19; r18 is sediment.
