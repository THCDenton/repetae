# Discovery Prompt v2.6 — per-source conventions interview (Stage 0.5)
...
- `[mechanical: <method>]` — counted/scripted against the source. **The
  method rides INSIDE the bracket, in one clause.** (Caption pairings,
  anchor tables, censuses, the watchlist.) Examples:
  `[mechanical: 21 matches]` · `[mechanical: Class: = Function: = 172, exact]`
  · `[mechanical: pdffonts — InvisibleOCR only]`. A bare `[mechanical]` with
  the method in the line's prose is a defect: the method must be
  machine-extractable from the tag, not recoverable by reading the sentence.
  This is the ONE form; v2.5 permitted both and said so nowhere, which made
  the tag unlintable [pinned v2.6 — see Version note].
...
Status: conventions ratified <date>, discovery_prompt_v2.6.
